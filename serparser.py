...
# Description                                                                                      #
#--------------------------------------------------------------------------------------------------#
# Handle and parse SER file                                                                        #
#--------------------------------------------------------------------------------------------------#
# History                                                                                          #
#--------------------------------------------------------------------------------------------------#
# coding 2026.01.26: 1st coding                                                                    #
#--------------------------------------------------------------------------------------------------#
...
class Serfile():
    ...
    def readTrailFromHeader(self):
        ...
        image_height = int(self._header['ImageHeight'])
        image_width  = int(self._header['ImageWidth'])
        frame_count  = int(self._header['FrameCount'])
        pixel_size   = int(self._bytesPerPixels)

        offset = image_height * image_width * frame_count * pixel_size + 178

        trail = []
        with open(self._nameOfSerfile, 'rb') as file:
            for i in range(self.getLength()):
                file.seek(offset)
                # trail.append(np.fromfile(file, dtype='uint64', count=1)[0]-621355968000000000) # 621355968000000000 = 0001-01-01 to 1970-01-01
                trail.append(np.fromfile(file, dtype='uint64', count=1)[0])
                offset+=8
        return trail
    ...
    def readFrameAtPos(self,n):
        ...
        if n<self._length : 
            with open(self._nameOfSerfile, 'rb') as file:
                ...
                self_frameDimension = int(self._frameDimension)
                self_bytesPerPixels = int(self._bytesPerPixels)

                offset = 178+n*self_frameDimension*self_bytesPerPixels
                file.seek(offset)
                if self._header['PixelDepthPerPlane']==8 :
                    frame=np.fromfile(file, dtype='uint8',count=self_frameDimension)
                else :
                    frame=np.fromfile(file, dtype='uint16',count=self_frameDimension)
